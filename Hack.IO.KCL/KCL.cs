...
namespace Hack.io.KCL
{
    ...
    public class WavefrontObj : List<Triangle>
    {
        ...
        public void MergeGroup(int srcIdx, int dstIdx)
        {
            if (srcIdx < 0 || srcIdx >= GroupNames.Count)
            {
                throw new IndexOutOfRangeException("Cannot remove group with index " + srcIdx + "; there are " + GroupNames.Count + " groups!");
            }
            if (dstIdx < 0 || dstIdx >= GroupNames.Count)
            {
                throw new IndexOutOfRangeException("Cannot merge group with index " + dstIdx + "; there are " + GroupNames.Count + " groups!");
            }
            ...
            {
                var tri = this[i];
                if (tri.GroupIndex == srcIdx)
                {
                    tri.GroupIndex = dstIdx;
                    this[i] = tri;
                }
                else if (tri.GroupIndex > srcIdx)
                {
                    tri.GroupIndex -= 1;
                    this[i] = tri;
                }
